...
class TestExercise(unittest.TestCase):
    ...
    def test_examples(self):
        ...
        self.assertEqual(solution( [2,7,11,15], target = 9),[1,2])
        self.assertEqual(solution( [2,3,4], target = 6),[1,3])
        self.assertEqual(solution( [-1,0], target = -1),[1,2])
...
class Solution:
    def twoSum(self, numbers: List[int], target: int) -> List[int]:
        ...
        while left < right:
            ...
            if tot == target:
                return [left+1,right+1]#python index starts in 0 not 1 - chaneg to 1-indexed
            elif tot < target:
                left += 1#increase left pointer - increase sum
            else:
                right -= 1#decrease right pointer - decrease sum
